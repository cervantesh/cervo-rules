...
| Vocabulary generator core | `internal/vocabgen/generator.go` | Load v3 vocabulary YAML and emit typed constants plus `Vocabulary()`. |
...
features such as `compat`, `inspect-api`, `diff`, `migrate-v3`, policy
`explain`, predicate composition, limits codegen, or derived facts codegen.
Those need separate v3 design and issues before being reintroduced.
